...
public class GameOfLife {
	...
	private static void test2(String fileName) {
		...
		for (int i = 1; i < board.length - 1; i++) {
			for (int j = 1; j < board[0].length - 1; j++) {
				System.out.printf("cell[%d][%d] - Neighbors count: %d, Original cell value: %d, new cell value: %d%n",
				                   i, j, count(board, i , j), board[i][j], cellValue(board, i, j));
			}
		}
	}
	...
	public static int[][] read(String fileName) {
		...
		for (int i = 1; i < board.length - 1 && !in.isEmpty(); i++) {
			int charCount = 0;
			String line = in.readLine();
			for (int j = 1; j < board[0].length - 1 && charCount < line.length(); j++) {
				if (line.charAt(charCount) == 'x') {
					board[i][j] = 1;
				}
				charCount++;
			}
	...
	public static int[][] evolve(int[][] board) {
		...
		for (int i = 1; i < evolvedBoard.length - 1; i++) {
			for (int j = 1; j < evolvedBoard[0].length - 1; j++) {
